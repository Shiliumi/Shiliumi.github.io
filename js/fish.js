(function () {
    var RENDERER = {
        ...
        watchWindowSize: function () {
            ...
            this.watchIds.push(window.setTimeout(this.jdugeToStopResize, this.WATCH_INTERVAL));
        },

        clearTimer: function () {
            while (this.watchIds && this.watchIds.length > 0) {
                window.clearTimeout(this.watchIds.pop());
            }
        },
        ...
        controlStatus: function () {
            ...
            for (var j = 0, pointCount = this.points.length; j < pointCount; j++) {
    ...
    FISH.prototype = {
        ...
        controlStatus: function (context) {
            this.previousY = this.y;
            this.x += this.vx * this.TIME_SCALE;
            this.y += this.vy * this.TIME_SCALE;
            this.vy += this.ay * this.TIME_SCALE;

            if (this.renderer.reverse) {
                if (this.y > this.renderer.height * this.renderer.INIT_HEIGHT_RATE) {
                    this.vy -= this.GRAVITY * this.TIME_SCALE;
                    this.isOut = true;
                } else {
                    if (this.isOut) {
                        this.ay = this.getRandomValue(0.05, 0.2);
            ...
            if (!this.isOut) {
                this.theta += Math.PI / 20 * this.TIME_SCALE;
                this.theta %= Math.PI * 2;
                this.phi += Math.PI / 30 * this.TIME_SCALE;
                this.phi %= Math.PI * 2;
            }
    ...
    window.StackPinkFish = {
        start: function () { RENDERER.init(); },
